...
class TaskAnalyzer {
  static Future<void> analyzeCategory({
    ...
    try {
      ...
      if (connectivityResult == ConnectivityResult.none) {
        onError('Отсутствует подключение к интернету. Пожалуйста, выберите категорию вручную');
        return;
      }
  ...
  static Future<void> analyzeEmotionalLoad({
    ...
    try {
      final connectivityResult = await Connectivity().checkConnectivity();
      if (connectivityResult == ConnectivityResult.none) {
        onError('Отсутствует подключение к интернету. Пожалуйста, выберите эмоциональную нагрузку вручную');
        return;
      }
      ...
      if (sentiment == null) {
        onError('Не удалось определить эмоциональную нагрузку. Пожалуйста, выберите значение вручную');
        return;
      }
  ...
  static void _handleError(dynamic error, Function(String) onError) {
    if (error is SocketException) {
      onError('Сервер недоступен. Проверьте подключение и выберите параметры вручную 🌐');
    } else if (error is TimeoutException) {
      onError('Сервер не отвечает. Пожалуйста, выберите параметры вручную ⏳');
    } else {
      final errorMessage = error.toString().split(':').first;
      if (errorMessage.contains('Exception')) {
        onError('Произошла ошибка при анализе. Выберите параметры вручную 😕');
      } else {
        onError('Что-то пошло не так. Пожалуйста, выберите параметры вручную 🔄');
      }
    }
  }
}